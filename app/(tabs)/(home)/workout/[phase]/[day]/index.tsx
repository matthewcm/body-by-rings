...
export default function WorkoutScreen() {
    ...
    const handleFinishPress = () => {
        const summary = Object.values(performanceLog).map(p => {
            ...
            const avgIntensity = (p.sets.reduce((sum, s) => sum + (parseFloat(s.intensity) || 0), 0) / setsCount).toFixed(1);
            return {
                name: p.exerciseName,
                summary: `${setsCount} sets, avg ${avgReps} reps @ ${avgIntensity}kg`
            };
        }).filter(Boolean);
    ...
    const confirmAndSaveWorkout = async () => {
        ...
        const finalLog = {
            ...
            performance: Object.values(performanceLog)
                .map(p => ({ exerciseName: p.exerciseName, notes: p.notes, sets: p.sets.filter(s => s.reps && s.reps.trim() !== '')}))
